...
const hostPage4 = async (req, res) => {
  try {
    ...
    // Render the page 4 view with the retrieved dogs
    return res.render('page4', { title: 'All Dogs', dogs: docs });
  } catch (err) {
    // Log the error and send back error message
    console.log(err);
...
const setDog = async (req, res) => {
  // Check if name, breed, and age are provided
  if (!req.body.name || !req.body.breed || !req.body.age) {
    return res.status(400).json({ error: 'name, breed, and age are all required' });
  }

  // Set up dog data based on the request body
  const dogData = {
    name: req.body.name,
...
// Function to find a dog by name and increase its age if found
const searchDog = async (req, res) => {
  // Check if the user provided a name as a query parameter
  if (!req.body.name) {
  ...
  try {
    ...
    let doc = await Dog.findOne({ name: req.body.name }).exec();

    // If the dog is not found, return an error
    if (!doc) {
    ...
    // Return the updated dog information to the client
    return res.json({
      name: doc.name,
      breed: doc.breed,
      age: doc.age,
...
// A function to send back 404 page
